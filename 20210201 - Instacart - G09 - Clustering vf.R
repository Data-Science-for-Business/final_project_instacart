...
colnames(ProjectData_with_kmeans_membership)<-c("User_id","Cluster_Membership_k")

head(ProjectData_with_kmeans_membership)

user_id_k_means <- (ProjectData_with_kmeans_membership)
user_id_k_means

user_id_k_means_df <- as.data.frame((user_id_k_means))
user_id_k_means_df$user_id <- as.factor(user_id_k_means_df$user_id) 
user_id_k_means_df$Cluster_Membership_k <- as.factor(user_id_k_means_df$Cluster_Membership_k) 

user_id_k_means_df_vF <- user_id_k_means_df
View(user_id_k_means)
...
df_users2 <- filter(df_users,row_number()<15000)
...
# -> nothing can be analysed with this
...
df_product2 <- filter(df_products,row_number()<15000)
